...
> **Architecture rule:** The nine sub-skills in this directory (`intro/`, `domain/`, `orient/`, etc.) are separate loadable files by design. The orchestrator (`../SKILL.md`) routes to one at a time. Do NOT merge them into a single file — that would defeat the routing architecture and force every invocation to load all layers. Each file is a tool grabbed when needed, not an encyclopedia to lug around.
